...
const TractorListing = ({
  ...
  const itemListSchema = itemsForSchema.length > 0 ? {
    ...
    "itemListElement": itemsForSchema.map((tractor, i) => {
      const position = i + 1 + (currentPage - 1) * itemsPerPage;
      ...
      return {
        "@type": "ListItem",
        "position": position,
        // Note: item is WebPage, not Product — avoids triggering Product snippet checks
        "item": {
          "@type": "WebPage",
          "url": url,
          "name": name
  ...
  return (
    <>
      {itemListSchema && (
        <Script id="tg-itemlist-schema" type="application/ld+json">
