...
const TractorListing = ({
  ...
  // --- Build ItemList node (ItemList -> ListItems with item -> WebPage) ---
  const itemListNode = itemsForSchema.length > 0 ? {
    ...
    "itemListElement": itemsForSchema.map((tractor, i) => {
      const pos = i + 1 + (currentPage - 1) * (itemsPerPage || 1);
      const url = abs((currentLang === 'hi' ? '/hi' : '') + (tractor?.page_url || ''));
      const name = `${tractor?.brand || ''} ${tractor?.model || ''}`.trim() || tractor?.page_url || `item-${pos}`;
      return {
        "@type": "ListItem",
        "position": pos,
        "item": {
  ...
  // --- Build Review nodes: one Review object per itemForSchema ---
  // Required-ish fields for Google review snippet: itemReviewed, author (name), reviewRating.ratingValue
  const reviewNodes = itemsForSchema.map((tractor, i) => {
    const position = i + 1 + (currentPage - 1) * (itemsPerPage || 1);
    const itemUrl = abs((currentLang === 'hi' ? '/hi' : '') + (tractor?.page_url || ''));
    const itemName = `${tractor?.brand || ''} ${tractor?.model || ''}`.trim() || tractor?.page_url || `item-${position}`;

    // ratingValue fallback: use avg_review if present, else 5 (safe default)
    const ratingValue = (tractor?.avg_review ?? tractor?.avgRating ?? tractor?.rating ?? null);
    const rating = ratingValue !== null ? String(ratingValue) : "5";

    // review body fallback: try tractor.short_description or a neutral default
    const reviewBody = tractor?.short_description || tractor?.excerpt || `Review for ${itemName}`;

    // datePublished fallback: use currentDate prop if available else today
    const datePublished = (tractor?.updated_at || tractor?.date || currentDate || new Date().toISOString()).split('T')[0];

    // author fallback: if you have reviewer name in data use it, otherwise generic
    const authorName = tractor?.review_author || tractor?.first_reviewer || 'TractorGyan User';

    // Build single Review node
    return {
      "@type": "Review",
      ...
      // itemReviewed points to the page (WebPage) — avoids declaring Product schema
      "itemReviewed": {
        "@type": "WebPage",
        "url": itemUrl,
        "name": itemName
  ...
  // --- Combine into a single @graph (context + graph of nodes) ---
  const graph = [];
  if (itemListNode) graph.push(itemListNode);
  // add review nodes (only if we have them)
  if (reviewNodes && reviewNodes.length > 0) reviewNodes.forEach(n => graph.push(n));
  ...
  return (
    <>
      {/* Server-rendered JSON-LD containing ItemList + Review nodes.
          This is the authoritative markup Google/SEMrush will read. */}
      {jsonLd && <script type="application/ld+json" dangerouslySetInnerHTML={{ __html: jsonLd }} />}

      {/* Visible UI (kept unchanged, WITHOUT product microdata attrs) */}
