...
const TractorListing = ({
  ...
  // prepare items for schema: use same slicing logic as UI (respect reel)
  const itemsForSchema = (reel ? (initialTyres || []).slice(showReelAfter) : (initialTyres || []));

  // Build JSON-LD object (minimal-safe: item -> WebPage)
  ...
  // Safe JSON string for embedding in script tag (escape closing script tag)
  const jsonLdSafeString = itemListSchema ? JSON.stringify(itemListSchema).replace(/<\/script>/gi, '<\\/script>') : null;

  return (
    <>
      {/* 1) Server-rendered JSON-LD script (crawler will see this in HTML) */}
      {jsonLdSafeString && (
        <script
          type="application/ld+json"
          // eslint-disable-next-line react/no-danger
          dangerouslySetInnerHTML={{ __html: jsonLdSafeString }}
        />
      )}

      {/* 2) Hidden microdata fallback (server-rendered): ensures itemListElement exists in HTML */}
      {itemsForSchema.length > 0 && (
        <div
          itemScope
          itemType="https://schema.org/ItemList"
          // hide visually but keep in DOM for crawlers
          style={{ display: 'none' }}
        >
              ...
              <>
                ...
                {reel ? (
                  <div
                    ...
                    itemScope
                    itemType="https://schema.org/ItemList" /* harmless, visible structure */
                  >
